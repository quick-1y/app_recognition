...
Полностью обновлённый пайплайн для детекции и OCR автомобильных номеров на основе YOLO и EasyOCR или CRNN с настраиваемым использованием GPU/CPU и локальной SQLite-базой для хранения событий и списков.

## Основные изменения
- **Новая конфигурация** (`config.yaml`) с секциями для модели, OCR, обработки, базы данных и путей к видео. Переключение между EasyOCR и CRNN задаётся параметром `ocr.backend`, переключение GPU/CPU для OCR вынесено в параметр `ocr.gpu`.
- **OCR**: поддержка EasyOCR (по умолчанию) и CRNN. Для CRNN используйте обученные веса с алфавитом и размером входных изображений, настроенными через конфиг.
...
1. Установите зависимости проекта:
   ...
   ```
2. Отредактируйте `config.yaml` под свои камеры/файлы и параметры OCR/YOLO. Для CRNN укажите путь к весам `ocr.crnn_weights` и при необходимости измените алфавит и размер входных изображений. Чтобы использовать EasyOCR, оставьте `ocr.backend: easyocr` и при необходимости скорректируйте список языков `ocr.languages`.
3. Запустите GUI:
   ```bash
   python run.py
   ```

Если окно сразу закрывается без ошибок, проверьте, что зависимости из `requirements.txt` установлены в активном окружении.
## Конфигурация
Пример `config.yaml`:
```yaml
app:
...
- **EasyOCR**: оставьте `ocr.backend: easyocr` и при необходимости измените список языков в `ocr.languages`.
- **CRNN OCR**: используйте обученные веса в `ocr.crnn_weights`, при необходимости скорректируйте `ocr.alphabet` и размеры входа.
- **Переключение GPU**: измените `ocr.gpu` на `true`, чтобы использовать GPU в EasyOCR или CRNN (при наличии).
