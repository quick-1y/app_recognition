...
Полностью обновлённый пайплайн для детекции и OCR автомобильных номеров на основе YOLO и собственной CRNN-модели с настраиваемым использованием GPU/CPU и локальной SQLite-базой для хранения событий и списков.

## Основные изменения
- **Новая конфигурация** (`config.yaml`) с секциями для модели, OCR, обработки, базы данных и путей к видео. Настройки GPU/CPU для OCR вынесены в параметр `ocr.gpu`.
- **OCR**: единый стек на CRNN. Используйте обученные веса с алфавитом и размером входных изображений, настроенными через конфиг.
...
1. Установите зависимости проекта:
   ...
   ```
2. Отредактируйте `config.yaml` под свои камеры/файлы и параметры OCR/YOLO. Для CRNN укажите путь к весам `ocr.crnn_weights` и при необходимости измените алфавит и размер входных изображений.
3. Запустите GUI:
   ```bash
   python run.py
   ```
