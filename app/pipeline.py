...
from app.detector import Detection, PlateDetector
from app.ocr import PlateCandidate, PlateRecognitionService
...
class PlatePipeline:
    def __init__(self, settings: Settings, database: PlateDatabase):
        ...
        self.detector = PlateDetector(settings)
        self.plate_recognizer = PlateRecognitionService(settings)
    ...
    def _annotate(self, frame: np.ndarray, box: tuple[int, int, int, int], class_name: str, color, label: str) -> None:
        x1, y1, x2, y2 = box
        cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
        if label:
            cv2.putText(frame, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)

    def _process_plate(self, frame: np.ndarray, bbox: tuple[int, int, int, int], source: str) -> Optional[PlateCandidate]:
        x1, y1, x2, y2 = bbox
        ...
        while True:
            ...
            for detection in detections:
                ...
                if class_name == "licence":
                    candidate = self._process_plate(frame, detection.bbox, source=video_path)
                    if candidate:
                        label = f"{candidate.text} {candidate.region}".strip()
                        callbacks.text(label)

                if self.settings.processing.draw_tracks:
                    self._annotate(frame, detection.bbox, class_name, color, label or class_name)
